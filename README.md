...
> i. This cloud-init.yaml template installs open source software dependencies of FreePBX (including Asterisk) from official Ubuntu LTS repositories. All packages installed from official Ubuntu repositories get security patches for 12 years when an Ubuntu Pro token is specified.
> ii. Ubuntu Pro is available for free for personal use or commercial evaluation purposes, on up to 5 Ubuntu installations.
